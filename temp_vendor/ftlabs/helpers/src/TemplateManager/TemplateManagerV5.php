 ...
 *
 * TODO:SH:20140723: Add codingstandards tag
...
class TemplateManagerV5 {
		...
		if (is_string($a) and is_scalar($b)) {
			...
			foreach ($b as $key=>$val) {
				...
				elseif (!is_scalar($val)); // Do nothing
					...
					throw new TemplateManagerException('Invalid template data assignment', get_defined_vars());
				}
			}
	...
	function render($templatename = false) {
		...
		if ($templatename) {
			...
			// Otherwise, work out the location of the template file on disk.
				...
				if (!$tempath) {
					throw new TemplateManagerException("Template '$templatename' not found in any valid location", get_defined_vars());
				}

				// Get the file contents and also keep in the cache for future reads
				$temstr = @file_get_contents($tempath);
				if (!$temstr) {
					throw new TemplateManagerException("Failure opening template file '$tempath'", get_defined_vars());
				}
			...
			throw new TemplateManagerException('No template specified', get_defined_vars());
		}
		...
		while (true) {
			...
			if (!$prefix) {
				...
				if (is_array($op) and is_array($op[0])) {
					throw new TemplateManagerException("Cannot use template collection '$key' outside a BLOCK", get_defined_vars());
				}
			...
			// Start of a block
				...
				if (isset($blocks[$key])) {
					throw new TemplateManagerException("Cannot nest block '$key' inside itself", get_defined_vars());
				}
				if (empty($key)) {
					throw new TemplateManagerException('No key specified for block', get_defined_vars());
				}
				if (!($endblock=strpos($blocks[$cb]["str"], "<!--@ENDBLOCK:".$key."-->", $pos))) {
					throw new TemplateManagerException("BLOCK specified with no matching ENDBLOCK for block $key", get_defined_vars());
				}
	...
	static function modifier_dateformat($ip, $format) {
		$op = CommonV3::convertHumanTime($ip);
	...
	static function modifier_timepast($ip, $spanclass=false, $titleformat=false, $rawformat=false, $cutoff=false) {
		$time = CommonV3::convertHumanTime($ip);
		if (!$time) return $ip;
		$now = new DateTime();
	...
	static function modifier_encode_javascriptvariable($ip) { return CommonV3::jsentities($ip); }
	static function modifier_encode_xml($ip) { return CommonV3::xmlentities($ip); }
