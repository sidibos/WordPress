...
class Thumbnail {
	...
	function Thumbnail($fckphp_config,$type,$cwd) {
		...
		if (!is_file($this->real_cwd) && is_file(urldecode($this->real_cwd))) $this->real_cwd = urldecode($this->real_cwd);
	...
	function run() {
		...
		// Assanka change - if the file wasn't found, use a generic icon
		} elseif (!file_exists($fullfile)) {
			$icon=iconLookup(false, false);
		} else {
