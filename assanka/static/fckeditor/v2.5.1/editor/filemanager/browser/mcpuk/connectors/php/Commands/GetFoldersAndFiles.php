...
<?php
			$files=array();
			if (!is_dir($this->real_cwd) && is_dir(urldecode($this->real_cwd))) $this->real_cwd = urldecode($this->real_cwd);
			...
			// Assanka change:WV:20071122:Sort files by name.
			sort($files);
			...
			for ($i=0;$i<sizeof($files);$i++) {
				...
				if (in_array(strtolower($ext),$this->fckphp_config['ResourceAreas'][$this->type]['AllowedExtensions'])) {
					...
					if (!$hide) {
						...
						echo "\t\t<File name=\"".htmlentities($files[$i], ENT_COMPAT, "UTF-8")."\" size=\"".ceil(filesize($this->real_cwd."/".$files[$i])/1024)."\" editable=\"" . ( $editable?"1":"0" ) . "\" />\n";	
					}
	...
	function isImageEditable($file) {
		// Assanka change: suppress errors as we're checking output correctly.
		$fh=@fopen($file,"r");
		if ($fh) {
			$start4=fread($fh,4);
			fclose($fh);
