/**
 * $Id: editor_plugin.js,v 1.2 2008-12-01 17:28:44 andrew Exp $
 *
 * @author Moxiecode
...
(function() {
	...
	tinymce.create('tinymce.plugins.SpellcheckerPlugin', {
		...
		init : function(ed, url) {
			...
			t.languages = {};
			each(ed.getParam('spellchecker_languages', '+English=en-gb,Danish=da,Dutch=nl,Finnish=fi,French=fr,German=de,Italian=it,Polish=pl,Portuguese=pt,Spanish=es,Swedish=sv', 'hash'), function(v, k) {
				if (k.indexOf('+') === 0) {
					k = k.substring(1);
					t.selectedLang = v;
				}
		...
		_getSeparators : function() {
			...
			// Assanka change: Question mark is not escaped in this list of separators
		...
		_markWords : function(wl) {
			...
			// Assanka change: This doesn't work, because it replaces partial words
			...
			each(nl, function(n) {
				...
				if (n.nodeType == 3) {
					...
					if (r1.test(v) || r2.test(v) || r3.test(v) || r4.test(v)) {
						...
						// Assanka change: This does work
						v = v.replace(r1, '$1<span class="mceItemHiddenSpellWord">$2</span>$3');
						v = v.replace(r2, '<span class="mceItemHiddenSpellWord">$1</span>');
						v = v.replace(r3, '<span class="mceItemHiddenSpellWord">$1</span>$2');
						v = v.replace(r4, '$1<span class="mceItemHiddenSpellWord">$2</span>');
