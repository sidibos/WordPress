    ...
    if ($controller) {
      ...
      if ($method) {
        ...
        if ($method == '404') {
		  ...
		  // Check if you can access the API (correct key):
		  if (!$this->canAccess($_REQUEST['api_key'])) {
  ...
  function admin_options() {
    ...
    if (!empty($_REQUEST['_wpnonce']) && wp_verify_nonce($_REQUEST['_wpnonce'], "update-options")) {
      ...
      if (isset($_REQUEST['json_api_base'])) {
		...
		$this->save_option(
			'json_api_require_key',
			(isset($_REQUEST['json_api_require_key']) ? (int)$_REQUEST['json_api_require_key'] : '0')
		);

		if (isset($_REQUEST['json_api_keys'])) {
			$rawKeys =  array();
			foreach(explode(PHP_EOL, $_REQUEST['json_api_keys']) as $rawKey) {
				$rawKey = trim($rawKey);
				if ($rawKey) {
					$rawKeys[] = $rawKey;
				}
			...
			$this->save_option('json_api_keys', implode(PHP_EOL, $rawKeys));
		}
...
<div class="wrap">
  ...
  <form action="options-general.php?page=json-api" method="post">
    ...
    <h3>Address</h3>
    <p>Specify a base URL for JSON API. For example, using <code>api</code> as your API base URL would enable the following <code><?php bloginfo('url'); ?>/api/get_page/?slug=path/to/page</code>. If you assign a blank value the API will only be available by setting a <code>json</code> query variable. <strong>Do not change this value unless you know what you are doing.</strong></p>
    ...
    <?php } ?>
	  ...
	  <?php
	  	$requireKeyChecked = $this->requireKey() ? 'checked="checked"' : '';
	  	$displayAuthorizedKeys = $this->requireKey() ? '' : ' style="display:none;"';
		$keys = get_option('json_api_keys', '');
	  ?>
	  <h3>API Keys</h3>
	  <p>If disabled, anyone can access the API</p>
	  <table class="form-table">
		  <tr valign="top">
			  <th scope="row">Require API Key</th>
			  <td><fieldset>
					  <legend class="screen-reader-text"><span>Require key to access the API</span></legend>
					  <label for="json_api_require_key">
						  <input name="json_api_require_key" type="checkbox" id="json_api_require_key" value="1" <?php echo $requireKeyChecked; ?> onClick="jQuery('#authorized_keys').toggle(this.checked);" />
						  Require key to access the API.
					  </label><br>
				  </fieldset>
			  </td>
		  </tr>
		  <tr valign="top" id="authorized_keys" <?php echo $displayAuthorizedKeys; ?>>
			  <th scope="row">Authorized keys</th>
			  <td>
				  <p>
					  Specify each authorized key on a new line.
					  Each key must be at least 32 characters long and cannot contain any spaces (all other characters are accepted).
					  Everything after a space character is considered a comment and ignored.
					  Valid format: <code>Secure-API-Key-at-least-32-chars <span style="color:#999999;">Comment for key 1</span></code>
				  </p>
				  <textarea name="json_api_keys" rows="3" class="large-text code"><?php echo $keys; ?></textarea>
				  <?php
				  	$invalidKeys = $this->getInvalidKeys();
				  	if (count($invalidKeys) > 0):
				  ?>
					  <p style="color: red;">
						  <strong>Warning: The following keys are invalid and will not be accepted:</strong> <br/>
						  <?php foreach($invalidKeys as $invalidKey): ?>
							  <code><?php echo $invalidKey; ?></code><br />
						  <?php endforeach; ?>
					  </p>
				  <?php endif; ?>
				  <p>
					  Random unique valid key: <code><?php echo bin2hex(openssl_random_pseudo_bytes(32)); ?></code>
				  </p>
			  </td>
		  </tr>
	  </table>
...
<?php
  ...
  function include_value($key) {
	...
	public function requireKey() {
		return (bool)get_option('json_api_require_key', false);
	}

	protected function getRawKeys() {
		return get_option('json_api_keys', '');
	}

	public function canAccess($checkKey = '') {
		// Check if we require keys at all
		if (!$this->requireKey()) {
			return true;
		}

		try {
			// Check if the provided $checkKey has valid format
			$checkKey = $this->getValidKey($checkKey);
		} catch (Exception $e) {
			return false;
		}

		// Go through all authorized keys and check them against the provided $checkKey
		foreach(explode(PHP_EOL, $this->getRawKeys()) as $rawKey) {
			try {
				if ($checkKey === $this->getValidKey($rawKey)) {
					return true;
				}
			} catch (Exception $e) {
				continue;
		...
		// None of the authorized keys matched the provided $checkKey
		return false;
	}
	...
	protected function getValidKey($rawKey) {
		list($key, $comment) = explode(' ', trim($rawKey), 2);
		if ($this->isKeyValid($key)) {
			return $key;
		}

		throw new Exception('Invalid key: ' . $rawKey);
	}

	protected function isKeyValid($key) {
		return (strlen(trim($key)) > 31);
	}
