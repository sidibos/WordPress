...
class JSON_API_Post {
  ...
  function save($values = null) {
    ...
    if (!empty($values['authors'])) {
      $author = $json_api->introspector->get_author_by_login($values['authors']);
      $wp_values['post_authors'] = $author->id;
    }
  ...
  function import_wp_object($wp_post) {
    ...
    $this->set_tags_value();
    $this->set_authors_value( function_exists( 'get_coauthors' ) ? get_coauthors($this->id) : array($wp_post->post_author) );
  ...
  function set_authors_value($authors) {
    global $json_api;
    if ($json_api->include_value('authors')) {
      if (!is_array($authors)) {
        $authors[] = $authors;
      }
      foreach ($authors as $author) {
        $this->authors[] = new JSON_API_Author($author->ID);
      }
    } else {
      unset($this->authors);
    }
  ...
  function set_thumbnail_value() {
    ...
    $this->main_image = $json_api->introspector->get_attachment($attachment_id);
  }
