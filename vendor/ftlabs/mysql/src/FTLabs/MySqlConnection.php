...
class MySqlConnection {
	...
	private $maxConnectionFailures = 2;
		...
		if (!($this->conn = mysqli_init()) instanceof mysqli) {
			...
			do {
				...
				if (!$this->conn->options(MYSQLI_OPT_CONNECT_TIMEOUT, 2)) {
					...
					if (++$failedConnectionAttempts == $this->maxConnectionFailures) {
	...
	private function runQuery($queryExpr) {
		...
		if (getmypid() !== $this->pid) {
			...
			// If this error is reached, the MySQL connection must have been created in the parent PHP process, while the query is being executed in a child process. This is a known cause of MySQL error 2006: 'MySQL server has gone away', and should be avoided.
			trigger_error('Executing query from forked process', E_USER_NOTICE);
		}
	...
	public function query() {
		...
		do {
			...
			if (empty($queryExpr)) {
				...
				throw new MySqlQueryException("Query is empty: $error", array('eh:caller'=>1, 'context'=>get_defined_vars()));
			}
		...
		if (!$resultObject and $this->reconnectOnFail and ($resultDetails['errorNo'] == self::CR_SERVER_LOST or $resultDetails['errorNo'] == self::CR_SERVER_GONE)) {
			...
			if ($this->reconnectCount++ < $this->maxConnectionFailures) {
				sleep(2);
		...
		if (!$resultObject and ($resultDetails['errorNo'] == self::ER_LOCK_WAIT_TIMEOUT)) {
			...
			throw new MySqlLockTimeoutException('MySQL lock wait timeout', array('eh:caller'=>1, 'context'=>get_defined_vars()));
		}
		...
		if (!$resultObject and !$this->suppressErrors) {
			...
			throw new MySqlQueryException("$resultDetails[errorMsg] ($resultDetails[errorNo]) occured in query: $queryExpr", $resultDetails['errorNo'], array('eh:caller'=>1, 'context'=>get_defined_vars()));
		}
	...
	public function parse() {
		if (!func_num_args()) {
			throw new MySqlQueryException('No query specified', array('eh:caller'=>1));
		}
	...
	private function replacePlaceholder($match) {
		...
		if ($this->mode == 'sprintf') {
			if (!count($this->args)) {
				throw new MySqlQueryException('Not enough parameters', array('eh:caller'=>1, 'context'=>get_defined_vars()));
			}
			...
			if (!array_key_exists($key, $this->args)) {
				throw new MySqlQueryException("Can't find argument for '$key'", array('eh:caller'=>1, 'context'=>get_defined_vars()));
			}
			$value = $this->args[$key];
			$sprintfFormat = null;
		}
	...
	private function parseValue($key, $value, array $modifiers = null, $sprintfFormat = null) {
		...
		if (!is_null($value) && !is_scalar($value) && !(is_object($value) && method_exists($value, '__toString'))) {
			throw new MySqlQueryException('Can\'t convert value to string', array('eh:caller'=>1, 'context'=>get_defined_vars()));
		}
	...
	/**
	 * Set the maximum connection failures allowed before the class will give
	 * up and throw an exception.
	 * @param integer $count the new number of connections allowed to fail before throwing an exception
	 * @return void
	 */
	public function setMaxConnectionFailures($count) {
		$this->maxConnectionFailures = $count;
	}
