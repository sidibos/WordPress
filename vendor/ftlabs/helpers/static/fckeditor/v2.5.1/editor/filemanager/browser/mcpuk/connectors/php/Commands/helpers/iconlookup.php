...
function iconLookup($mime,$ext) {
	...
	// First, retrieve the given URL for this script.
	$assankaPath = dirname($_SERVER['PHP_SELF']);
	
	// Strip off the corestatic path
	$assankaPath = substr($assankaPath, strlen("/corestatic/"));
	
	// Add the core folder location on this server
	$assankaPath = $_SERVER["CORE_PATH"]."/static/".$assankaPath;
