...
// 			$_FILES['NewFile']['name']=$_FILES['NewFile']['name']; //$this->cleanFilename($_FILES['NewFile']['name']);
		...
		if (sizeof($_FILES)>0) {
			...
			// Assanka change: disable file size checks - lots of commented-out lines beneath with //>
			// Assanka change: also trap upload errors at this stage.
			if (array_key_exists("NewFile",$_FILES) and !$_FILES["NewFile"]["error"]) {
			...
			//>	if ($_FILES['NewFile']['size']<($typeconfig['MaxSize']*1024)) {
					...
					// Assanka change:20081203:Remove all dots apart from the final one (see support request #5939)
					$filename = str_replace(".", "", substr($filename, 0, $lastdot)).substr($filename, $lastdot);
					$lastdot=strrpos($filename,".");
								...
								if (file_exists($this->real_cwd."/$filename.$ext")) {
									...
									if (is_uploaded_file($_FILES['NewFile']['tmp_name'])) {
										if (move_uploaded_file($_FILES['NewFile']['tmp_name'],($this->real_cwd."/$filename.$ext"))) {
											...
											$disp="202,'Failed to upload file, internal error...'";
											trigger_error("Outline - Failed to move uploaded file to ".$this->real_cwd."/".$filename.".".$ext, E_USER_NOTICE); // Assanka change
										}
									} else {
										if (rename($_FILES['NewFile']['tmp_name'],($this->real_cwd."/$filename.$ext"))) {
											...
											$disp="202,'Failed to upload file, internal error...'";
											trigger_error("Outline - Failed to move file to ".$this->real_cwd."/".$filename.".".$ext, E_USER_NOTICE); // Assanka change
										}
									}
			//>					}
							}
							...
							$disp="202,'This filetype is not permitted in this folder; if you are uploading a normal file, please ensure you are not uploading through the Image dialog.'";
						}
			...
			//>	}
				...
				// Assanka change: detect and report upload errors
				if ($_FILES["NewFile"]["error"]) {
					switch ($_FILES["NewFile"]["error"]) {
						case 1:
							$disp="202,'The file you uploaded was too large, and was therefore not accepted.  Try saving your file in a format more suited to the web, as users will otherwise take some time to download the file.  For example, for images, save as a JPEG with some compression.'";
							break;					
						case 2:
							$disp="202,'The file you uploaded was too large, and was therefore not accepted.  Try saving your file in a format more suited to the web, as users will otherwise take some time to download the file.  For example, for images, save as a JPEG with some compression.'";
							break;					
						case 3:
						case 4:
							$disp="202,'The file you uploaded was only partially received, possibly due to a network problem.  Please try again.'";
							break;
						default:
							trigger_error("An unexpected error occurred when user attempted to upload a file (>4)", E_USER_NOTICE);
							$disp="202,'Unable to process uploaded file due to an unexpected error.'";
					}					
				} else {
