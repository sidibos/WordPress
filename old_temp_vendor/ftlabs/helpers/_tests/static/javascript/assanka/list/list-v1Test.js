buster.testCase('CircularList', {
	'Append should append objects to the end of the list' : function() {
		...
		var obj = {};
		var itemA = new CircularList.Item(obj)
		var itemB = new CircularList.Item(obj + {"o": 1});
	...
	'Remove should remove an item from the linked list' : function() {
		...
		var itemA = new CircularList.Item({A:1}),
			itemB = new CircularList.Item({B:2}),
			itemC = new CircularList.Item({C:2});
		...
		list.remove(new CircularList.Item({C:{C:2}}));
	...
	'To array should return an array representation of the circularly linked list' : function() {
		...
		var itemA = new CircularList.Item({A:1}),
			itemB = new CircularList.Item({B:2}),
			itemC = new CircularList.Item({C:3});
	...
	'Insert an item after an already exisiting item in the linked list' : function() {
		...
		var itemD = new CircularList.Item({D:4});
	...
	'CircularList#destroy should unset properties on member items which are not explicitly a CircularList.Item object' : function() {
		var list = new CircularList();
